...
describe('Accessibility', () => {
  ...
  describe('Saturation Keyboard Navigation', () => {
    it('updates saturation on ArrowRight/Left', async () => {
      ...
      await interactive.trigger('keydown', { key: 'ArrowRight' })
      expect((wrapper.emitted('change')?.[0]?.[0] as any).s).toBe(51)
      
      await interactive.trigger('keydown', { key: 'ArrowLeft' })
      expect((wrapper.emitted('change')?.[1]?.[0] as any).s).toBe(49)
    })
    ...
    it('updates brightness on ArrowUp/Down', async () => {
      ...
      expect((wrapper.emitted('change')?.[0]?.[0] as any).v).toBe(51)
      
      await interactive.trigger('keydown', { key: 'ArrowDown' })
      expect((wrapper.emitted('change')?.[1]?.[0] as any).v).toBe(49)
    })
  })
  ...
  describe('ARIA attributes', () => {
    it('has correct ARIA roles and labels', () => {
      ...
      expect(wrapper.find('.vue3-colorful__hue [role="slider"]').attributes('aria-label')).toBe('Hue')
      expect(wrapper.find('.vue3-colorful__alpha [role="slider"]').attributes('aria-label')).toBe('Alpha')
    })
  })
})
